function scrollableFrameViewer_2p0_x_y(data,xdata,ydata,startFrame,colorMapUse)
    % Create sample data
    frameIdx = startFrame;
    ...
    ax = axes('Parent', fig);
    hImg = pcolor(ax, xdata,ydata, data(:,:,frameIdx)); shading interp
...
function keyPressHandler(fig, event)
    switch event.Key
        ...
        case 'g'
            getptsActive = getappdata(fig, 'getptsActive');
            % if getptsActive
            %     disp('Already collecting points.');
            %     return;
            % end

            setappdata(fig, 'getptsActive', true);
            % setappdata(fig, 'cancelGetPts', false);
            % disp('Point selection started. Press "n" to stop.');

            % Start point selection asynchronously
            drawnow;
            collectPoints(fig);
    end
end
